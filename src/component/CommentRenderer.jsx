...
const CommentRenderer = () => {
    ...
    const addComment = () => {
        if (commentOrReplay.trim()) {
            ...
            addNoti(post_id, commentOrReplay, post_info?.postOwner._id);
            setCommentOrReplay("")
    ...
    const doReplay = () => {
        ...
        addNoti(post_id, commentOrReplay, commentWoner);
        setCommentOrReplay("")
        notify("replay added ☺");
    }
    ...
    function addNoti(commentId_, commentText, receiverId) {
        axios.post(server_port + "/api/addNoti/add", { receiverId, senderId: localStorage.getItem("myId"), type: "comment", commentId: commentId_, text: commentText, postId: null })
    }
                    ...
                    duration-200 hover:text-lime-300'
                                            ...
                                            <div>{rep.replay.map((innerRep, index) => (
                                                <div key={index} className='border-s border-b ml-1 sm:ml-8 p-2 rounded-sm'>
                                                    ...
                                                    <div>{innerRep?.replay?.map((nest, index) => (<div key={index} className='p-0 '>
                                                        <hr />
