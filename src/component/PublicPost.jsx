...
const PublicPost = () => {
    ...
    return (
        <div className='sm:p-4 h-auto' id='postInnerContainer' >
            <ToastContainer />
            {/* <div className={`border ${isSave ? "flex" : "hidden"}`}>
                <span className='flex cursor-pointer' onClick={() => { save(idForSave); setIsSave(false) }}><Save /> save post</span>
            </div> */}
            {
                posts.map((data, index) => (
                    <div key={index} className={`mx-auto w-full  rounded-lg border p-2 backdrop-blur-md my-5 ${postBG} ${data.video ? "h-[600px]" : "h-auto"}`}>
                        ...
                        {data.image && !data.video ? <img className='rounded-md w-full max-h-96 object-fill' src={server_port + data.media} /> : <video src={server_port + data.media} controls loop className='h-[450px] w-full object-fill'></video>}
                        <footer className='flex w-full justify-between items-end border-t h-10'>
                            ...
                            <span className='post_footer w-4/12 border-e justify-center'
                                ...
                                <MessageSquareIcon />
                                <span>comments {formatNumber(data.comments.length)}</span>
                            </span>
            ...
            <div className={`w-full h-full backdrop-blur-sm fixed top-0 left-0 flex justify-center items-center ${isComment ? "" : "hidden"} z-50`} id='commentplate'>
                <div className="w-full sm:w-8/12 rounded-md bg-teal-950 h-full sm:h-[92vh] shadow-xl p-2">
                    ...
                    <div className='w-full h-[82vh] sm:h-[75vh] overflow-y-auto'>
                        ...
                        hover:text-cyan-500'><ShortText text={data.user.name} width={window.innerWidth} dot={3} /></Link>
                                            </div>
                                            <span>|</span>
                ...
                hover:text-orange-500 cursor-pointer'
                                                onClick={() => { document.getElementById(`replayPlate${index}`).style.display = "block" }}><ShortText text={"view replay"} width={window.innerWidth} dot={2} /></span>

                                        </div>
                                        <h5 className='ml-6 mt-1'>{data.text}</h5>
                                    </div>
                                    <div className={`ml-1 mt-0 hidden`} id={`replayPlate${index}`}>{data.replies.map((rep, index) => (
                    ...
                    duration-200 hover:text-lime-300'
                                            ...
                                            <div>{rep.replay.map((innerRep, index) => (
                                                <div key={index} className='border-s border-b ml-1 sm:ml-8 p-2 rounded-sm'>
                                                    ...
                                                    <div className=''>{innerRep.replay.map((nest, index) => (<div key={index} className='p-0 '>
                                                        <hr />
